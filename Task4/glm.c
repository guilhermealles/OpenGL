    ...
    while (node) {
      if (node->averaged) {
        ...
        if (T(node->index).vindices[0] == i)
            T(node->index).nindices[0] = avg; else if (T(node->index).vindices[1] == i)
            T(node->index).nindices[1] = avg; else if (T(node->index).vindices[2] == i)
        ...
        if (T(node->index).vindices[0] == i)
            T(node->index).nindices[0] = numnormals; else if (T(node->index).vindices[1] == i)
            T(node->index).nindices[1] = numnormals; else if (T(node->index).vindices[2] == i)
...
glmSpheremapTexture(GLMmodel* model) {
  ...
  for (i = 1; i <= model->numnormals; i++) {
    ...
    if(r == 0.0) {
      ...
      if(z == 0.0)
        phi = 3.14159265 / 2.0; else
        phi = acos(z / rho);
      if(y == 0.0)
        theta = 3.141592365 / 2.0; else
...
glmDraw(GLMmodel* model, GLuint mode) {
  ...
  if (mode & GLM_COLOR)
      glEnable(GL_COLOR_MATERIAL); else if (mode & GLM_MATERIAL)
...
GLvoid glmDrawVBO(GLMmodel* model, GLuint mode) {
  ...
  //glDrawElements(GL_TRIANGLES, 3*model->numvertices, GL_UNSIGNED_BYTE, NULL);
